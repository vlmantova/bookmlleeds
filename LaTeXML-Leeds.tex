...
WIP: integrate steps 1-2 into \verb|latexmlleeds|, so that \lstinline|\usepackage[tikzexternalize]{latexmlleeds}| has the same effect without changes to the \verb|.tex| code.
...
\verb|latexmlleeds| makes \LaTeXML{} include \verb|latexmlleeds.css| (described below) and it defines \ltxinline|\ldsHTML{html}|, which outputs the \ltxinline|html| argument directly as \HTML{} code. The \ltxinline|html| is only processed by \LaTeXML{} and ignored by other engines. The \LaTeXML{} processing is done by \verb|latexmlleeds.sty.lxtml|, which is a short Perl file you do not need to touch.
\begin{exa}
  Code:
  \begin{lstlisting}[style=latexml]
