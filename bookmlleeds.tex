...
\date{7\textsuperscript{th} January 2026}
...
\begin{foldedframe}[Full list of prerequisites]
  \begin{itemize}
    ...
    \item Support for EPS, PDF images: Ghostscript and possibly mutool
...
\begin{tabularx}{\textwidth}[h]{>{\color{MidnightBlue}\tt}r@{\texttt{\ }}>{\tt}X}
  ...
  mutool:        & \textcolor{red}{NOT FOUND} (may be required for PDF to SVG)                   \\
  dvisvgm:       & \textcolor{OliveGreen}{3.4.3 OK} (required for BookML images, EPS/PDF to SVG) \\
  dvisvgm/libgs: & \textcolor{OliveGreen}{9.25 OK} (required for BookML images, EPS/PDF to SVG)  \\
...
\textbf{To update BookML}, run \lstinline|make check-for-update|, then \cmdinline|make update|, simply replace the \texttt{bookml} folder with the content of a newly downloaded \href{https://github.com/vlmantova/bookml/releases/latest/download/bookml.zip}{\texttt{bookml.zip}}.
