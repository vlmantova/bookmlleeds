...
\date{20\textsuperscript{th} March 2026}
...
\section{Running BookML via Overleaf and/or GitHub}\label{running-bookml-via-overleaf-and-or-github}

\subsection{Overleaf}\label{overleaf}

\begin{foldedframe}[1.\ Link Overleaf with GitHub]
  Open your Overleaf \href{https://www.overleaf.com/user/settings}{account settings} and \href{https://docs.overleaf.com/integrations-and-add-ons/git-integration-and-github-synchronization/github-synchronization#linking-your-overleaf-account-to-your-github-account}{link your GitHub account} from the `GitHub Sync' entry.
\end{foldedframe}

\begin{foldedframe}[2.\ Set up a new or existing project]
  \begin{itemize}
    \item Open an existing or new Overleaf project and start \href{https://docs.overleaf.com/integrations-and-add-ons/git-integration-and-github-synchronization/github-synchronization#creating-a-new-github-repository-from-an-overleaf-project}{synchronizing it with a new GitHub repository}. \textbf{Warning:} choose a repository name without spaces! Inscrutable and hard-to-fix errors will ensue otherwise.
    \item Visit the repository page and click the `Watch' icon (next to `Edit Pins', `Fork', `Star'). \textbf{If you skip this step, you will not get the emails} with the links to download the compiled files.
    \item Add the file \texttt{.github/workflows/bookml.yaml} to the project with the following content:
...
jobs:
  build:
    ...
    steps:
      ...
      \end{lstlisting}
    \item Now synchronize the project with GitHub. GitHub will ask you to grant permissions to Overleaf to run workflows. Say yes! After a few minutes, you will receive an email with the results, provided you `watched' the repository.
    \item \textbf{Note:} in the default configuration, BookML will try to compile all \texttt{.tex} files at the \textbf{top folder} of your repository that contain the string \ltxinline|\documentclass| (even if commented out!). The work is done by the BookML Docker image, which at the moment contains a full copy of \TeX{} Live 2021.
  \end{itemize}
\end{foldedframe}

\begin{foldedframe}[3.\ Recompile a file]
  \begin{itemize}
    \item Simply open the `Integrations' sidebar, click on `GitHub', and push your changes to GitHub.
    \item If you have `watched' the repository, you will receive an email after a few minutes with the links to download the files, and/or any error messages.
    \item Otherwise, visit the repository linked to the project and look under the `Actions' tab. Once the latest run completes it will get a green tick or a red cross. Open the run and scroll to the bottom to find where to download the files, and any relevant error messages.
    \item \textbf{Note:} each run creates a new `release', which you can find under the `Releases' heading on the front page of the repository. Releases will remain available forever unless you intentionally delete them.
  \end{itemize}
\end{foldedframe}

\subsection{Directly on GitHub}\label{directly-on-github}
If you do not use Overleaf, you can still use GitHub to compile your files. This assumes that you already know how to use GitHub, and that you have your \texttt{.tex} files in a GitHub repository.

\begin{foldedframe}[1.\ Set up an existing repository]
  \begin{itemize}
    \item Visit the repository page and click the `Watch' icon (next to `Edit Pins', `Fork', `Star'). \textbf{If you skip this step, you will not get the emails} with the links to download the compiled files.
    \item Add the file \texttt{.github/workflows/bookml.yaml} to the project with the following content:
      \begin{lstlisting}[style=yaml,name=bookml.yaml]
...
\begin{foldedframe}[Linux Debian-based (Ubuntu, Debian, Mint, etc)]
  ...
  \begin{lstlisting}[language=bash]
sudo dpkg -i latexml_0.8.8-4_all.deb
sudo apt -f install
...
\begin{foldedframe}[Linux School PC]
...
module use /apps/linsw1/modulefiles/7/
  \end{lstlisting}
  Please request IT assistance if you do not have a \texttt{latexml} module, and request an update if you are still on an old version of \LaTeXML{} (that is, before 0.8.8).
\end{foldedframe}

\begin{foldedframe}[Windows (AppsAnywhere --- easiest)]
  \textcolor{red}{\textbf{Changed on January 2026.}} Due to a subtle (and baffling) issue with CloudPaging Player, the instructions below are a little different now.
  \begin{itemize}
    \item Launch \href{https://appsanywhere.leeds.ac.uk/}{AppsAnywhere} (see \href{https://it.leeds.ac.uk/it?id=kb_article&sysparm_article=KB0014827}{IT Knowledge Base} if you have issues).
    \item Install Ghostscript, ImageMagick, MiK\TeX{}, StrawberryPerl.
    \item Open a terminal and install mutool:
      \begin{lstlisting}
winget install mutool
      \end{lstlisting}
    \item \textbf{Only on first install.} Open a terminal, or StrawberryPerl, and run:
      \begin{lstlisting}
llw32helper.bat
      \end{lstlisting}
      Answer `y' to the first question, and just press Enter on the second question (or choose a different folder if you wish to). \textbf{Warning:} running \cmdinline|llw32helper| a second time will ask you about \emph{undoing} what it just did. Only run it once!

      Now close and reopen the terminal and continue with the following.
      \begin{lstlisting}
cpanm --build-args=CC=c++ --notest --verbose Image::Magick
cpanm --notest --verbose LaTeXML
      \end{lstlisting}
  \end{itemize}
  Everything else should then be available. Re-do the \cmdinline|cpanm| steps to update. You may have to reinstall the apps every one or two months (open the CloudPaging Player to check their status), but you will not need to reinstall \LaTeXML{}.
\end{foldedframe}
...
\subsection{Adding BookML to a new or existing folder}\label{adding-bookml}
...
\begin{tabularx}{\textwidth}[t]{>{\color{MidnightBlue}\tt}r@{\texttt{\ }}>{\tt}X}
...
\subsection{Converting your files}\label{converting-your-files}
...
\subsection{Minimum recommendations}\label{minimum-recommendations}
...
\subsection{Alternative formats}\label{alternative-formats}
...
\section{Uploading to Minerva}\label{uploading-to-minerva}
\subsection{SCORM packages}\label{scorm-packages}
\begin{enumerate}
...
\subsection{Plain ZIP packages}\label{plain-zip-packages}
