...
llw32helper.bat
      \end{lstlisting}
      Answer `y' to the first question, and just press Enter on the second question (or choose a different folder if you wish to). Then close and reopen the terminal before continuing.
      \begin{lstlisting}
cpanm --build-args=CC=c++ --notest --verbose Image::Magick
cpanm --notest --verbose LaTeXML
      \end{lstlisting}
  \end{itemize}
  Everything else should then be available. Re-do the \cmdinline|cpanm| steps to update. You may have to reinstall the apps every one or two months (open the CloudPaging Player to check their status).
\end{foldedframe}
