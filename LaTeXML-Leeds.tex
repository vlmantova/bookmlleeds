...
\end{lxFooter}
\fancyfoot[L]{Copyright \copyright{} 2021 Vincenzo Mantova, University of Leeds.}
\fancyfoot[C]{}
\fancyfoot[R]{\thepage}
\pagestyle{fancy}
...
zip -r mynotes.zip mynotes
  \end{lstlisting}
  Do create a subfolder (the \verb|mynotes| in \verb|mynotes/myfile.html| above) since there are additional files that need to travel with the main \verb|myfile.html|. (The option \verb|--urlstyle=file| is necessary if any of your files is called \verb|index.html| because of how Minerva serves \HTML{} files.)
  \item You can now upload the zip file on Minerva using the `Upload zip package' feature. You must ask Minerva overwrite the existing files or the package won't update properly.
\end{enumerate}
...
\Large Some large text here   % DON'T
    \end{lstlisting}
  \item (MathJax restriction.) Do not put images within a mathematical environment \ltxinline|$...$| or \ltxinline|$$...$$|, unless it is the only content. If you absolutely need the images, disable MathJax for the particular equation with \ltxinline|\bmlDisableMathJax|, or for the entire document with the option \ltxinline|nomathjax|, and ask your students to use Safari or Firefox.
    \begin{lstlisting}[style=latexml]
$$ \includegraphics{image.pdf} $$          % not recommended but ok
$$ \includegraphics{image.pdf} \qedhere $$ % DON'T
    \end{lstlisting}
  \item (temporary) Do not put references within a mathematical environment.
    \begin{lstlisting}[style=latexml]
\[ x = y \text{ by Proposition \ref{x=y}} \] % DON'T, for now
    \end{lstlisting}
\end{itemize}
...
\end{lxFooter}
  ...
  \item[Disable MathJax for an equation] Add \ltxinline|\bmlDisableMathJax| within the equation. Remember that the output will only work with Firefox and Safari, though.
